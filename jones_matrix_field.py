...
class AntennaFarFieldResponse(object):
    ...
    def derive_symmetric_rotated_feed(self, rotation_angle_sign='positive'):
        ...
        if rotation_angle_sign == 'negative':
            ...
            raise ValueError("The parameter rotation_angle_sign must be 'negative' or 'positive'.")
    ...
    def interpolate_spatial_harmonics_spectra(self, nu_axis):
        ...
        zph = self.zenith_phi
        delta = pyssht.generate_dl(np.pi/2., self.L_model)
        ...
        for ii in range(nu_axis.size):
            ...
            zen_Eabs[ii] = np.sqrt(np.abs(zen_Et)**2. + np.abs(zen_Ep)**2.)
        ...
        if self.dual_feed == True:
            ...
            for ii in range(nu_axis.size):
                ...
                zen_rEabs[ii] = np.sqrt(np.abs(zen_rEt)**2. + np.abs(zen_rEp)**2.)
    ...
    def compute_spatial_spline_approximations(self, nu_axis, L_synth='model'):
        ...
        if L_synth < self.L_model:
            ...
            raise ValueError("The synthesized bandlimit (L_synth) must be higher than the bandlimit of the model (L_synth > L_model).")

        theta_axis, phi_axis = pyssht.sample_positions(L_synth, Method='MWSS', Grid=False)
        ...
        def spline_from_data(grid_data):
            ...
            spl = RectBivariateSpline(mu_axis_flip, phi_axis_pad, grid_data_use,
                                      bbox=[-1.,1.,0.,2*np.pi],
                                      kx=3,
                                      ky=3,
                                      s=0)
        ...
        def splines_from_harmonics(pos1_Elm, neg1_Elm, L_synth):
            ...
            pos1_E = pyssht.inverse(pos1_Elm_pad, L_synth, Spin=1, Method='MWSS', Reality=False)
            neg1_E = pyssht.inverse(neg1_Elm_pad, L_synth, Spin=-1, Method='MWSS', Reality=False)

            Et = (pos1_E + neg1_E)/np.sqrt(2.)
            Ep = (pos1_E - neg1_E)*(-1j)/np.sqrt(2.)
        ...
        self.E_spls = [splines_from_harmonics(p, n, L_synth) for (p,n) in zip(self.pos1_Elm, self.neg1_Elm)]
        self.rE_spls = [splines_from_harmonics(p, n, L_synth) for (p,n) in zip(self.pos1_rElm, self.neg1_rElm)]
        ...
        for ii in range(Nfreq):
            for aa in range(2):
                for bb in range(2):
                    ...
                    self.rE_spl_coeffs[ii,aa,bb] = self.rE_spls[ii][aa][bb].get_coeffs()

        def construct_component_functions(E_spl):

            def Et_func(theta, phi, grid=False):
                mu = np.cos(theta)
                return E_spl[0][0](mu, phi, grid=grid) + 1j*E_spl[0][1](mu, phi, grid=grid)

            def Ep_func(theta, phi, grid=False):
                mu = np.cos(theta)
                return E_spl[1][0](mu, phi, grid=grid) +1j*E_spl[1][1](mu, phi, grid=grid)
        ...
        def construct_directivity_function(Et_spl, Ep_spl):
            ...
            def D_func(theta, phi, grid=False):
                return np.abs(Et_spl(theta, phi, grid=grid))**2. + np.abs(Ep_spl(theta, phi, grid=grid))**2.
        ...
        self.D_funcs = [construct_directivity_function(t, p) for (t,p) in zip(self.Et_funcs, self.Ep_funcs)]
        self.rD_funcs = [construct_directivity_function(t, p) for (t,p) in zip(self.rEt_funcs, self.rEp_funcs)]
